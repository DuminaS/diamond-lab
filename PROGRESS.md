...
- ~~`renderBracketTreeSVG`~~ **REMOVED in Round 24, replaced by `renderPlayoffTreeSVG`** — see the Round 24 note below for the current, unified renderer (used by BOTH conferences now, including the player's own real path). `.standings-columns > *{min-width:0}` / `.bracket-wrap svg{max-width:100%; height:auto}` (Round 22, still current) fixed the Playoff Tree overflowing off-screen and remain load-bearing: (1) CSS Grid items don't shrink below their content's intrinsic width by default — a wide child inflating the whole `.standings-columns` grid track past the viewport is a well-known Grid gotcha, `min-width:0` on the grid children is the fix, and any future `.standings-columns`-style two-column layout hosting wide content needs the same rule; (2) an SVG's `width`/`height` ATTRIBUTES (not CSS) set its actual rendered size regardless of container, so `max-width:100%; height:auto` in CSS is what actually lets it shrink to fit, using the `viewBox` to preserve internal layout/aspect ratio.
- `nodeMatchFromRealRound` / `playoffTreeConfDisplay` / `renderPlayoffTreeSVG` / `ensurePlayoffTreeReveal` (Round 24) — the current, unified Playoff Tree bracket system, replacing Round 21's `renderBracketTreeSVG` + the Round 20-22 reuse of `renderPlayoffBracketSVG` for the player's own conference (that mixing was a real, reported bug — two visually-clashing renderers on one tab). `nodeMatchFromRealRound(r)` maps ONE of the player's real `season.playoffs.rounds` entries into the exact same `{aSeed,aId,aScore,bSeed,bId,bScore,winnerId}` shape `resolveFullBracketWithRounds` already produces for a flat conference — this is what lets ONE renderer (`renderPlayoffTreeSVG`) draw both sides; if a future change ever alters either shape, keep them in sync or the unified renderer breaks for one side. `playoffTreeConfDisplay(conf, season, pb, myConf)` is the ONE place that decides which data source to read for a given conference (the player's live real path, the eager `_otherConfBracketCache`, or the final `pb` record) and returns a normalized `{rounds, isReal, championKnown, championId, revealedCount, totalRounds}` — any future Playoff Tree feature should go through this function rather than reading any of those three sources directly. `career.playoffTreeReveal = {year, AFC, NFC}` (via `ensurePlayoffTreeReveal`) is the NEW "Simulate Next Round" reveal-counter state — transient, current-season-only (like `career.currentSeasonSchedules`), and deliberately NEVER applies to the player's own real conference (that side is always paced by the existing Season-tab Key Moment reveal; `playoffTreeConfDisplay`'s `isMyRealConf` branch is what enforces this). `season.leagueStandings._otherConfBracketCache` (Round 24) is a NEW eager resolve of the other conference, computed immediately in `resolvePlayoffs` rather than waiting for `finalizeLeaguePlayoffBracket` (which only runs once the player's own fate is known) — `finalizeLeaguePlayoffBracket` now reuses this exact cache instead of re-resolving, since a second `resolveFullBracketWithRounds` call would roll fresh `Math.random()` results and could disagree with whatever was already revealed on the tab. `resolveFullBracketWithRounds` itself now also generates a score per matchup (`approxGameScore`) — it previously only recorded a winner, which was fine for the Round 20 champion-only use case but insufficient once the renderer/box-score modal needed real scores to display. **Round 25 correction**: the original Round 24 version of `renderPlayoffTreeSVG` stacked every round's boxes by plain row index with no relationship to their feeders, and connected them with a smooth bezier that could cut straight through an unrelated box — both real, reported bugs. The current version uses the standard bracket-layout technique instead: every seed gets a stable initial vertical slot, and each later round's box Y is the AVERAGE of its two feeding seeds' current positions (tracked seed-by-seed across rounds via a `seedPos` map) — a future change to the pairing logic in `resolveFullBracketWithRounds` must keep `aSeed`/`bSeed` correct on every matchup or this alignment breaks. Connectors are now right-angle "elbow" paths (`H`/`V`/`H` path commands), never a diagonal curve, specifically because an elbow's only vertical segment sits entirely within the empty gap between columns and so can never cross through a box. `renderPlayoffTreeSVG` also gained a `mirrored` param (flips which side each round renders on — round 0 near the outer edge, the final round near the center — without changing the chronological seed-bookkeeping order) so the "other" conference converges toward the shared column boundary from the opposite direction of "my" conference, rather than both running left-to-right independently; `buildPlayoffTreeTabHTML` passes `mirrored=true` only for `otherConf`. Note this mirroring only reads as "converging" in the 2-column desktop layout (`.standings-columns` at `min-width:640px`) — on a narrow single-column mobile layout the mirrored conference's round order is simply reversed with no particular visual benefit, an accepted minor inconsistency given the fix's scope, not a bug to chase further without a specific report.
- `estimateSingleGameStatLine(qb)` / `buildBracketBoxScoreModalHTML` (Round 24) — the Playoff Tree's box-score modal. A flat-resolved playoff matchup (nobody real involved) never had a per-game QB stat line, only a final score (see above) — `estimateSingleGameStatLine` distributes that QB's REAL season per-game average with natural variance rather than fabricating something disconnected from his actual year. For the player's own real games, the modal uses the ALREADY-REAL `match.realRound.box` (from the pre-existing `generateGameBoxScore`, called when the round was first created) instead — never re-estimates a real game. Quarter-by-quarter splits (for both real and flat matches, for rendering consistency) reuse `distributeAcrossGames` (Round 20) directly. The modal reuses the pre-existing, previously-UNUSED generic `.modal-overlay`/`.modal-box`/`.modal-head`/`.modal-close` CSS pattern (confirmed via grep it had zero other call sites before this) — any future new modal in this app should check that pattern first before building a bespoke one.
...
- `buildWeekMatchups(season, week)` / `weekMatchupTeamLineHTML` (Round 22) — replaced the Round 20 team-picker Schedule tab with a real week-by-week board (pick a week, see every real game across the league that week), per explicit user clarification that they wanted this to work like a real NFL schedule page. Weaves the player's own REAL game (`season.gameLog`) into the board alongside every other team's shared-schedule game (`career.currentSeasonSchedules`) for that week. IMPORTANT, a deliberate and still-open architectural boundary: the player's own regular season has ALWAYS been simulated independently of the shared league-wide schedule (predates this round entirely) -- when the player's real opponent's OWN shared-schedule entry for that week would show a DIFFERENT, fictitious game, `buildWeekMatchups` just SKIPS that team's entry for the week rather than showing a contradiction, so that team simply looks like it had a bye that week instead of visibly disagreeing with itself. This is a narrow, low-visibility trade-off, not a fix for the underlying disconnect -- a future round that wants the player's own season to be truly, fully woven into the shared schedule (not just cosmetically patched at display time) would need to unify `pickRegularSeasonOpponents`/`simulateRegularSeasonGames` with `buildScheduleResults`/`scheduleGamesIntoWeeks` into one real pass, a materially bigger change than this one. **Round 26 correction**: the original skip condition (`if(!g || seen.has(g.opponentId)) return;`) was too broad — it dropped a team's own card whenever its INTENDED OPPONENT happened to already be `seen`, which could cascade well past the single coincidental player-vs-shared-schedule conflict this was meant to paper over, silently vanishing unrelated third-party teams from the week board entirely. Narrowed to `if(seen.has(id)) return;` (only skip if THIS team's own card is already rendered) plus `if(!seen.has(g.opponentId)) seen.add(g.opponentId);` (only suppress the opponent's separate card if not already claimed) — a team can now appear paired with two different opponents across two cards in a rare conflict week, but never disappears outright. Any future change to this function must preserve that a team's own `id` is the ONLY thing gating whether ITS card renders — never gate on its opponent's claimed status.
