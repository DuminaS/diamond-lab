...
- **Deferred to Round 7**: the player's own bench/depth-chart mechanic (starting as a backup behind an entrenched incumbent at draft time, a real "clipboard" season with reduced stats/development, and a job-competition roll to win the starting job) was scoped alongside this round's opponent-side work but not yet built. When picked up, the draft-night handler (`enterDraftNightBtn` click listener) is the right insertion point for the incumbent check — it already has `team`/`leagueStrength`/`decade`/`slot` in scope at the exact moment `career.teamId` is first set, before `generateLeagueRivals()` runs.
