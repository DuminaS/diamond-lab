...
- **Playoff Tree bracket system — CURRENT architecture as of Round 32** (superseded, in order: Round 21's `renderBracketTreeSVG` → Round 24's unified `renderPlayoffTreeSVG` → Round 28's combined-SVG `renderFullPlayoffTreeSVG` → Round 29's CSS Grid `renderPlayoffBracketGrid` (unchanged visually since) → Round 32's lockstep data model underneath it; only this entry describes the current system, the dated round entries above have the full history of why each rewrite happened). The RENDERER (`renderPlayoffBracketGrid`/`bracketColumnHtml`/`bracketCardHtml`/`bracketSuperBowlColumnHtml`) is UNCHANGED from Round 29 — same CSS Grid, same card states. What changed is where its input data comes from: `playoffTreeConfDisplay(conf, season)` (signature simplified — no more `pb`/`myConf` params, both conferences are named on `season.leagueStandings.bracket` itself) now reads straight off the ONE shared lockstep record instead of splicing a real result into an independently-rolled flat cache — see the Round 32 entry for the full architecture (`bracket.myState`/`otherState`, `myRounds`/`otherRounds`, `myChampionId`/`otherChampionId`, `stepBracketConferenceOnce`, `confirmAndRecordRound`). A round is either fully confirmed (present in `myRounds`/`otherRounds`) or hasn't happened yet at all — there is no more "pending-known" (rolled-but-not-shown) state, since lockstep makes contradiction structurally impossible rather than papering over it after the fact. `canonicalRoundLabels(N, wcGames, byes)` is a new pure function giving the not-yet-reached columns their correct header label without resolving anything. The Playoff Tree also no longer lives in its own dash-tab as of Round 32 — `buildPlayoffTreeTabHTML`'s output renders directly inside `tabpanel-season` (the `#tabpanel-playofftree` element id is unchanged, just relocated in the DOM) — any future change to the season-card template must keep it there, not reintroduce a separate `data-tab="playofftree"` button. `.standings-columns`/`confSectionHTML` are NOT used for the Playoff Tree tab (removed Round 28) — a future change must not reintroduce two separate per-conference render calls, since that structural choice is what caused the Round 28 label-collision bug in the first place.
