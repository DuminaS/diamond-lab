...
- `resolveFullBracketWithRounds(seeds, format)` / `finalizeLeaguePlayoffBracket(season, myPlayoffs)` (Round 20, internals reworked Round 29 — read this version, not the original) — a real, permanent champion for both conferences and the Super Bowl, every season, previously undetermined unless the player personally won it. `finalizeLeaguePlayoffBracket` has exactly two call sites and must keep having exactly those two: inside `resolvePlayoffs` (immediately, when `mySeedIdx===-1` — the player missing the playoffs is known instantly) and inside `finalizeRound`'s "whole run is done" branch, right alongside the pre-existing `finalizePlayoffOutcome(season)` call (the player making the playoffs isn't fully resolved until their own reveal, Key Moments included, actually finishes). A future third path that can end a player's playoff involvement (there isn't one today) would need this call added too, or that season's `leagueStandings.playoffBracket` silently never gets set. **Round 29 rework**: `resolveFullBracketWithRounds`'s round-resolution logic is now factored into two shared helpers, `playBracketRound(rounds, pairs, label)` (resolves one round, appends it, returns survivors) and `continueBracketRounds(rounds, field)` (loops `playBracketRound` down to a single champion, re-seeding highest-vs-lowest-remaining each round) — both are reused by the NEW `resolveRemainingBracketWithRounds(field)`, which replaced the old champion-only `resolveRemainingBracketField(field)`. This exists because the player's own conference now gets a FULL flat bracket too (`season.leagueStandings._myConfBracketCache`, mirroring the pre-existing other-conference cache, for sibling-game display — see the Round 29 `playoffTreeConfDisplay` note below), and that cache has no idea what the player's real result was for any given round; `resolveRemainingBracketWithRounds` is what lets `finalizeLeaguePlayoffBracket` splice a CAUSALLY CONSISTENT continuation (starting from the player's real remaining field, with real round-by-round detail) into `myBracket.rounds` for everything after the player's last real round, instead of leaving the cache's own independently-rolled (and potentially contradictory) continuation in place. See the Round 29 PROGRESS.md entry for the exact bug this fixed (the player's team shown eliminated in one column, then winning the next) and how the splice index is found (by matching the player's last-played round LABEL, not just counting rounds played — a bye can make those differ).
...
- **Playoff Tree bracket system — CURRENT architecture as of Round 29** (superseded, in order: Round 21's `renderBracketTreeSVG` → Round 24's unified `renderPlayoffTreeSVG` → Round 28's combined-SVG `renderFullPlayoffTreeSVG` → Round 29's CSS Grid `renderPlayoffBracketGrid`; only this entry describes the current system, the dated round entries above have the full history of why each rewrite happened). `renderPlayoffBracketGrid(afcDisplay, nfcDisplay, pb, myTeamId)` draws a real `display:grid` bracket — AFC's columns left-to-right, NFC's mirrored right-to-left, one `bracket-col-sb` column dead center — with NO connector lines; convergence reads from column order plus each column's cards being flexbox-distributed (`justify-content: space-around`/`center`) across its own full height, which can never clip and needs no pixel math. `playoffTreeConfDisplay(conf, season, pb, myConf)` is still the one place that normalizes a conference's display data, but as of Round 29 BOTH conferences always use the SAME full-flat-bracket-plus-splice model: `season.leagueStandings._myConfBracketCache` (NEW, mirrors the pre-existing `_otherConfBracketCache`, eagerly resolved in `resolvePlayoffs` regardless of whether the player makes the playoffs) supplies sibling games for the player's own conference too, and `nodeMatchFromRealRound(r)` splices the player's real per-round result into whichever ONE matchup involves `career.teamId` — every OTHER matchup in that round keeps the independent flat roll. There is no more `isMyRealConf`/"real vs flat" branch — `fullRounds` (the complete, unsliced round list) and `rounds` (sliced to `revealedCount`) are always built the same way for both conferences. Reveal pacing: `revealedCount = max(realRoundsPlayed, clickCounter)`, so the player's own conference auto-reveals through however many rounds they've actually played (no click needed) and needs "Simulate Next Round" clicks for anything beyond that — including everything, for either conference, once the player's own run is fully over. `career.playoffTreeReveal = {year, AFC, NFC}` (via `ensurePlayoffTreeReveal`) is unchanged in shape, just now read/written uniformly for both conferences instead of only the flat one. See the Round 29 entry for the elimination-consistency bug this splicing approach initially introduced (the flat cache not knowing the player really lost, so it could keep advancing them in a later round's sibling context) and how `resolveRemainingBracketWithRounds` fixes it at the source. `.standings-columns`/`confSectionHTML` are NOT used for the Playoff Tree tab (removed Round 28) — a future change must not reintroduce two separate per-conference render calls, since that structural choice is what caused the Round 28 label-collision bug in the first place.
