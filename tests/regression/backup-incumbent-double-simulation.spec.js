// Confirmed live against HEAD (67b425c): resolveBackupSeasonSnaps() calls
// simulatePlayerSeasonStats(incumbent, ...) directly (pushing one season row onto
// incumbent.seasons and incrementing his totals/age), then later in the SAME generateSeason()
// call, simulateRivalSeasons() processes every non-retired rival -- including this same incumbent,
// who is deliberately NOT excluded (PROGRESS.md Round 7: "so he can retire naturally and open the
// job") -- calling simulatePlayerSeasonStats() on him again for the SAME year. This test forces
// career.isBackup=true on a real, drafted career, advances exactly one season, and asserts the
// incumbent has exactly one season row for that year (Section 3, invariant #6: no more than one
// (qbId, year) season record).
...
test("backup-incumbent-simulates-once-per-year", async ({ page }) => {
  ...
  const incumbentAfter = (after.career.leagueRivals || []).find(r => r.id === incumbent.id);
